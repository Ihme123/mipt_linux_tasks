...
void * get_shared_memory_block(const char *file, int *shmid_res) {
	int shmid;
	if ( (shmid = shmget(ftok(file, 1), 100, 0666|IPC_CREAT))<0 ) {
		printf("Can not get shmid\n");
		return (void*)-1;
	}
	if ( shmid_res )
		*shmid_res = shmid;
...
//	d->type = 0;
	...
	if(isDryer)
	{
		dd->smem = get_shared_memory_block(d->file, NULL);
		return (int) dd->smem;
	}
	else
	{
		dd->smem = get_shared_memory_block(d->file, &(dd->shmid));
		OOM(dd->smem)
...
int SMEMSend(struct connDesc *d, int data)
{
    ...
    if (data == EOC)
	{
		//we change the sign of quantyty to indicate that we won't add dishes anymore
		((int*)dd->smem)[1] = true;
		my_semop(dd->sem,0,-1);
		return 0;
	};
...
int SMEMRead(struct connDesc *d)
{
	...
	//printf("--READ!!!!! %d\n", Val);
	//printf("**Removed dish %d from place %d\n",*data,((int*)dd->smem)[0]+1);
	(((int*)dd->smem)[0])--;
	my_semop(dd->sem,0,-1);
